	...
	},
	{
		...
		render: record => (
			...
			<Select
				...
				defaultValue={['BASIC']}
				onSelect = {(selectedIndex) => showMessage("Please Wait...", `${roles[selectedIndex]}  was successfully assigned to ${record.colleague}`)}
				onDeselect = {(selectedIndex) => showMessage("Please Wait...", `${roles[selectedIndex]}  was successfully revoked from ${record.colleague}`)}
		  	>
			{roles.map((role, key)=>(
...
export const customerCols = [
	{
	  ...
	  },
	  {
		...
		key:'aum',
		render: data => (
				<CustomerContact contactData={data.contactInfo}/>
		)
	  },
	  {
		title: 'AUM',
	...
	},
	{
		...
		render: record => (
			...
			onSelect = {(selectedIndex) => {
				showMessage("Please Wait...", `${subdomainList[selectedIndex]}'s access was granted to ${record.colleague}`)}}
				onDeselect = {(selectedIndex) => showMessage("Please Wait...", `${subdomainList[selectedIndex]}'s access was revoked from ${record.colleague}`)}
		  >
...
export const byEmailModalConfig = {
	...
	content: (
	  <>
		We've sent validation emails to the following emails for approval to issue the certificate. Please make sure this process is completed within 72 hours (3 days) or the certificate request will be expired.
		...
		/>
		<Space/>
		You can also track the validation progress in <b>Manage Subdomains &gt; In Progress</b> 
	  </>
	),
	okText: "OK",
	onOk: () => {},
...
export const byDNSModalConfig = {
	...
	content: (
	  <>
		Add the following CNAME record to the DNS configuration for you domain. The porcedure for adding CNAME record depends on you DNS service Provider

		<Table className="mt-3" pagination={false} size="small" columns={dnsCols} dataSource={dnsData} scroll={{x:true}}/>
